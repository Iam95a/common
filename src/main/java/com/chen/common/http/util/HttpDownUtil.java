package com.chen.common.http.util;

import com.chen.common.http.boot.HttpDownBootstrap;
import com.chen.common.http.entity.HttpHeadsInfo;
...
public class HttpDownUtil {
  ...
  public static HttpResponseInfo getHttpResponseInfo(HttpRequest httpRequest, HttpHeaders resHeaders, ProxyConfig proxyConfig, NioEventLoopGroup loopGroup)
    ...
    if (resHeaders == null) {
      ...
      if ((httpResponse.status().code() + "").indexOf("30") == 0) {
        ...
        ProtoUtil.RequestProto requestProto = ProtoUtil.getRequestProto(requestInfo);
  ...
  public static HttpResponse getResponse(HttpRequest httpRequest, ProxyConfig proxyConfig, NioEventLoopGroup loopGroup) throws Exception {
    ...
    HttpRequestInfo requestInfo = (HttpRequestInfo) httpRequest;
    ProtoUtil.RequestProto requestProto = requestInfo.requestProto();
  ...
  public static ProtoUtil.RequestProto parseRequestProto(String url) throws MalformedURLException {
    return parseRequestProto(new URL(url));
  }

  public static ProtoUtil.RequestProto parseRequestProto(URL url) {
    int port = url.getPort() == -1 ? url.getDefaultPort() : url.getPort();
    return new ProtoUtil.RequestProto(url.getHost(), port, url.getProtocol().equalsIgnoreCase("https"));
  }
